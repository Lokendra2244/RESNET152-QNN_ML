...
# MODIFIED: Changed input file from train_set to test_set
db = pd.read_csv("./archive/csv/mass_case_description_test_set.csv")
...
# MODIFIED: Changed output filenames to reflect test data
if TEST_MODE:
    db.to_csv("clean_test_set.csv", index=False)
else:
    db.to_csv("clean_test_set_full.csv", index=False)

# %% cell 7: printing image grid to confirm the data is correct
# MODIFIED: Changed the verification loader to read the new test CSVs
csv_to_check = "clean_test_set.csv" if TEST_MODE else "clean_test_set_full.csv"
...
fig.suptitle("Data Pipeline Verification: Test Set Cropped Tissues", fontsize=16)
